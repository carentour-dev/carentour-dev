...
adoptable_links AS (
    SELECT
        ...
        ROW_NUMBER() OVER (
            ...
            ORDER BY nav.position NULLS LAST, nav.created_at NULLS LAST, nav.id
        ) AS row_number
    FROM public.navigation_links AS nav
    INNER JOIN published_pages AS page
        ON nav.slug = page.slug
    WHERE
        nav.cms_page_id IS NULL
        AND NOT EXISTS (
            SELECT 1
            FROM public.navigation_links AS existing_link
            WHERE existing_link.cms_page_id = page.id
        )
)

UPDATE public.navigation_links AS nav
SET
    kind = 'cms',
    cms_page_id = adoptable_links.cms_page_id,
    href = CASE
        WHEN adoptable_links.slug = 'home' THEN '/'
        ELSE '/' || adoptable_links.slug
    END,
    status = 'published',
    updated_at = NOW()
FROM adoptable_links
WHERE
    nav.id = adoptable_links.nav_id
    AND adoptable_links.row_number = 1;
