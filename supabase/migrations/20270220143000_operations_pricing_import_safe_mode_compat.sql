...
SET can_apply = false
WHERE can_apply IS NULL;

ALTER TABLE public.operations_pricing_import_runs
ALTER COLUMN can_apply SET DEFAULT false;

ALTER TABLE public.operations_pricing_import_runs
ALTER COLUMN can_apply SET NOT NULL;
...
CREATE TABLE IF NOT EXISTS public.operations_pricing_import_items (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    run_id UUID NOT NULL REFERENCES public.operations_pricing_import_runs (id) ON DELETE CASCADE,
...
UPDATE public.operations_pricing_import_runs AS runs
SET can_apply = (
    runs.blocking_count = 0
    AND EXISTS (
        SELECT 1
        FROM public.operations_pricing_import_items AS items
        WHERE items.run_id = runs.id
            AND items.status = 'ready'
    )
)
WHERE runs.status = 'preview';

DROP FUNCTION IF EXISTS public.apply_operations_pricing_import(UUID, UUID, UUID);
...
REVOKE ALL ON FUNCTION public.apply_operations_pricing_import(UUID, UUID, BOOLEAN)
FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.apply_operations_pricing_import(UUID, UUID, BOOLEAN)
TO service_role;
