...
function StartJourneyWizardContent({
  mode = "standalone",
  className,
  backHref = "/",
  introEyebrow = "Start Your Journey",
  introHeading = "Share your case and we will coordinate the next steps.",
  introDescription = "Complete the intake below to receive a tailored treatment and travel plan from the Care N Tour team.",
  supportCardTitle = "What happens after submitting?",
  supportCardDescription = "Our medical concierges coordinate surgeons, travel, and recovery in one itinerary.",
  supportBullets = defaultSupportBullets,
  responseTimeLabel = "Average response time: under 2 hours",
  reassuranceLabel = "No payment required to submit your intake",
  successRedirectHref = "/",
}: StartJourneyWizardProps) {
  ...
  return (
    <div
      className={cn(
        isEmbedded ? "bg-transparent" : "min-h-screen bg-background",
        className,
      )}
    >
      {isEmbedded ? (
        <div className="space-y-3 pb-8">
          {introEyebrow ? (
            <span className="inline-flex items-center rounded-full border border-border/70 bg-background/80 px-3 py-1 text-xs font-semibold uppercase tracking-[0.18em] text-muted-foreground">
              {introEyebrow}
            </span>
          ) : null}
  ...
  return (
    <div
      className={cn(
        ...
        embedded
          ? "min-h-[320px] rounded-[1.5rem] border border-border/60"
          : "min-h-screen",
      )}
    >
      <div className="text-center">
...
export function StartJourneyWizard(props: StartJourneyWizardProps) {
  const embedded = props.mode === "embedded";

  return (
    <Suspense fallback={<StartJourneyFallback embedded={embedded} />}>
      <StartJourneyWizardContent {...props} />
    </Suspense>
  );
}

export default StartJourneyWizard;
