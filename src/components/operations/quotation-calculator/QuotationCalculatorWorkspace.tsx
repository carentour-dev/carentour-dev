...
type QuotationCalculatorWorkspaceProps = {
  apiBasePath?: string;
  printBasePath?: string;
};

export function QuotationCalculatorWorkspace({
  apiBasePath = "/api/admin/operations",
  printBasePath = "/operations/quotation-calculator",
}: QuotationCalculatorWorkspaceProps = {}) {
  const { toast } = useToast();
  const invalidate = useAdminInvalidate();
  const defaultValues = useMemo(() => buildDefaultQuoteInput(), []);
  const buildApiPath = useCallback(
    (path: string) => `${apiBasePath}${path}`,
    [apiBasePath],
  );
  ...
  const pricingProvidersQuery = useQuery({
    ...
    queryFn: () =>
      adminFetch<ProviderSummary[]>(buildApiPath("/pricing/providers")),
    staleTime: 5 * 60 * 1000,
  });
  ...
  const providerPricingQuery = useQuery({
    ...
    queryFn: () =>
      adminFetch<ProviderProcedurePricingResponse>(
        buildApiPath(
          `/pricing/providers/${selectedProviderId}/procedure-price-lists`,
        ),
      ),
  ...
  const quotesQuery = useQuery({
    ...
    queryFn: () => adminFetch<OperationsQuote[]>(buildApiPath("/quotes")),
  });

  const createQuote = async (payload: QuoteInput) =>
    adminFetch<OperationsQuote>(buildApiPath("/quotes"), {
      method: "POST",
  ...
  const updateQuoteMutation = useMutation({
    mutationFn: async ({
      ...
      adminFetch<OperationsQuoteDetail>(buildApiPath(`/quotes/${quoteId}`), {
        method: "PUT",
        body: JSON.stringify(payload),
      }),
  ...
  const fetchExchangeRates = useCallback(
    (signal?: AbortSignal) => {
      return adminFetch<ExchangeRatesPayload>(buildApiPath("/exchange-rates"), {
        signal,
      });
    },
    [buildApiPath],
  ...
  const handlePrint = (quoteId?: string | null) => {
    if (!quoteId) return;
    window.open(`${printBasePath}/${quoteId}/print`, "_blank");
  };
