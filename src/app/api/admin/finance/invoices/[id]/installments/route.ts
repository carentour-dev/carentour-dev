...
const FINANCE_INVOICE_INSTALLMENTS_PERMISSIONS = {
...
export const GET = adminRoute(async (_req, ctx) => {
  ...
  const result = await financeController.getInvoiceInstallments(invoiceId);
  return jsonResponse(result);
}, FINANCE_INVOICE_INSTALLMENTS_PERMISSIONS);
...
export const PUT = adminRoute(async (req: NextRequest, ctx) => {
  ...
  const result = await financeController.replaceInvoiceInstallments(
    invoiceId,
    payload,
    {
      userId,
