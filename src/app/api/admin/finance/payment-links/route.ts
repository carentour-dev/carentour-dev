export const dynamic = "force-dynamic";

import { NextRequest } from "next/server";
import { adminRoute } from "@/server/utils/adminRoute";
import { ApiError } from "@/server/utils/errors";
import { jsonResponse } from "@/server/utils/http";
import { financeController } from "@/server/modules/finance/module";

const FINANCE_PAYMENT_LINK_PERMISSIONS = {
  allPermissions: ["finance.access", "finance.shared", "finance.invoices"],
} as const;

export const GET = adminRoute(async (req: NextRequest, ctx) => {
  const userId = ctx.auth?.user.id;
  if (!userId) {
    throw new ApiError(401, "Missing authenticated user");
  }

  const patientId = req.nextUrl.searchParams.get("patientId");
  const links = await financeController.listPaymentLinks({
    patientId: patientId && patientId.trim().length > 0 ? patientId : null,
  });

  return jsonResponse(links);
}, FINANCE_PAYMENT_LINK_PERMISSIONS);

export const POST = adminRoute(async (req: NextRequest, ctx) => {
  const userId = ctx.auth?.user.id;
  if (!userId) {
    throw new ApiError(401, "Missing authenticated user");
  }

  const payload = await req.json();
  const link = await financeController.createPaymentLink(payload, {
    userId,
    profileId: ctx.auth?.profileId ?? null,
    permissions: ctx.auth?.permissions ?? [],
  });

  return jsonResponse(link, 201);
}, FINANCE_PAYMENT_LINK_PERMISSIONS);
