...
export const GET = adminRoute(async (_req, ctx) => {
  const serviceProvider = await serviceProviderController.get(
...
export const PATCH = adminRoute(async (req: NextRequest, ctx) => {
  ...
  const serviceProvider = await serviceProviderController.update(
    getRouteParam(ctx.params, "id"),
    body,
  );
  return jsonResponse(serviceProvider);
...
export const DELETE = adminRoute(async (_req, ctx) => {
  const result = await serviceProviderController.delete(
    getRouteParam(ctx.params, "id"),
  );
