...
export async function POST(req: NextRequest) {
  try {
    ...
    const documentType = documentTypeSchema.parse(documentTypeInput);
    ...
    const documentSuffixParts = [
      slugify(baseName),
      patient ? slugify(patient.full_name) : "guest",
    ];
    if (patient) {
      documentSuffixParts.push(patient.id.slice(0, 8));
    }
    const randomSegment = randomUUID().slice(0, 8);
    documentSuffixParts.push(randomSegment);

    const storedFileName = `${documentSuffixParts.filter(Boolean).join("_")}${extension ? `.${extension}` : ""}`;
    const folder = patient?.id ?? "guest";
    const storagePath = `start-journey/${folder}/${storedFileName}`;
    ...
    console.error("[start-journey][documents][POST]", error);
    return NextResponse.json(
      {
        error:
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    console.error("[start-journey][documents][DELETE]", error);
    return NextResponse.json(
      {
        error:
          error instanceof z.ZodError
