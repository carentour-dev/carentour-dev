...
const formSchema = z.object({
  ...
  duration_minutes: z
    ...
    .refine(
      (value) =>
        value === undefined ||
        (Number.isInteger(value) && value >= 5 && value <= 480),
      {
...
export default function AdminConsultationsPage() {
  ...
  const contactRequestQuery = useQuery({
    ...
    queryFn: () =>
      normalizedContactRequestId
        ? adminFetch<ContactRequestRow>(
            `/api/admin/requests/${normalizedContactRequestId}`,
          )
  ...
  const renderLinkedRequestSummary = () => {
    ...
    return (
      <div className="mt-4 rounded-md border border-primary/30 bg-primary/5 p-4 text-sm">
        ...
        {request.message && (
          <p className="mt-3 line-clamp-3 text-sm text-muted-foreground whitespace-pre-line">
            {request.message}
          </p>
        )}
      </div>
  ...
  const syncLinkedRequest = useCallback(
    async (requestId: string, patientId: string) => {
      try {
        await adminFetch<ContactRequestRow>(
        ...
        toast({
          ...
          description:
            error instanceof Error
              ? error.message
              : "Unable to update the linked request status.",
          variant: "destructive",
        });
      }
    },
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="md:col-span-2">
                      ...
                      <FormControl>
                        <Input
                          placeholder="On-site clinic or virtual"
                          {...field}
                        />
                      </FormControl>
                      <FormMessage />
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Textarea
                        rows={4}
                        placeholder="Pre-consultation checklist, interpreter needs, etc."
                        {...field}
                      />
                    </FormControl>
                    <FormMessage />
